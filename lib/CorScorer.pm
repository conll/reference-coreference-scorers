...
sub Score {
	my ( $metric, $kFile, $rFile, $name, $allowMultipleTagging ) = @_;
	our $repeated_mentions = 0;
	my $allowMultiple =
	  ( ( $allowMultipleTagging == 1 ) && defined($allowMultipleTagging) )
	  ? 1
	  : 0;

	if ( lc($metric) eq 'blanc' ) {
		return ScoreBLANC( $kFile, $rFile, $name, $allowMultiple );
	}
	...
	if ( defined($name) && $name ne 'none' ) {
		...
		foreach my $iname ( keys( %{$kIndexNames} ) ) {
			my $keys =
			  GetCoreference( $kFile, $KEY_COLUMN, $iname,
				$kIndexNames->{$iname} );
			my $response =
			...
			my (
			  ...
			  = IdentifMentions( $keys, $response, $allowMultiple,
				\%idenTotals );
			my ( $nr, $dr, $np, $dp ) = Eval(
				$metric,                                $keyChains,
				$keyChainsWithSingletonsFromResponse,   $responseChains,
		...
		while ( my $l = <F> ) {
			...
			if ( $l =~ /\#\s*end document/ ) {
				foreach my $h (@half) {
					if ( defined($h) && @$h ) {
						die
						  "Error: some mentions in the document do not close\n";
					}
				}
				last;
			}
...
sub IdentifMentions {
	...
	foreach my $entity (@$keys) {
		foreach my $mention (@$entity) {
			if ( $allowMultiple
				&& defined( $id{"$mention->[0],$mention->[1]"} ) )
			{

				#record multiple instance joined by comma
				$id{"$mention->[0],$mention->[1]"} = join( ',',
					$id{"$mention->[0],$mention->[1]"},
					$id{"$mention->[0],$mention->[1]"} );
			}
			elsif ( defined( $id{"$mention->[0],$mention->[1]"} ) ) {
				print
				  "Repeated mention in the key: $mention->[0], $mention->[1] ",
	...
	foreach my $entity (@$response) {
		...
		foreach my $mention (@$entity) {
			if ( $allowMultiple
				&& defined( $map{"$mention->[0],$mention->[1]"} ) )
			{
				$map{"$mention->[0],$mention->[1]"} = join( ',',
					$map{"$mention->[0],$mention->[1]"},
					$map{"$mention->[0],$mention->[1]"} );
				
				if ( index($id{"$mention->[0],$mention->[1]"}, $map{"$mention->[0],$mention->[1]"}) == 0){
					$exact++;
				}
			}
			elsif ( defined( $map{"$mention->[0],$mention->[1]"} ) ) {
				print
"Repeated mention in the response: $mention->[0], $mention->[1] ",
				  $map{"$mention->[0],$mention->[1]"},
				  $id{"$mention->[0],$mention->[1]"},
				  "\n";
				push( @remove, $i );
...
"Found too many repeated mentions (> 10) in the response, so refusing to score. Please fix the output.\n";
				...
				&& !$assigned[ $id{"$mention->[0],$mention->[1]"} ] )
			{
				...
				if ( !$allowMultiple ) {
					...
					if (
						index( $id{"$mention->[0],$mention->[1]"}, ',' ) == -1 )
					{
						$map{"$mention->[0],$mention->[1]"} =
						  $id{"$mention->[0],$mention->[1]"};
					}
	...
	if ($VERBOSE) {
		...
		print "Total response mentions: " . $totalResponseMentions . "\n";
...
sub Eval {
	...
	if ( $scorer eq 'muc' ) {
		...
		( $nr, $dr, $np, $dp ) =
		  BCUBED( $keyChainsOrig, $responseChainsOrig, $allowMultiple );
	}
	elsif ( $scorer eq 'ceafm' ) {
		( $nr, $dr, $np, $dp ) =
		  CEAF( $keyChainsOrig, $responseChainsOrig, 1, $allowMultiple );
	}
	elsif ( $scorer eq 'ceafe' ) {
		( $nr, $dr, $np, $dp ) =
		  CEAF( $keyChainsOrig, $responseChainsOrig, 0, $allowMultiple );
	}
	else {
...
sub MUCScorer {
	my ( $keys, $keysPrecision, $response, $responseRecall, $allowMultiple ) =
	  @_;
	...
	foreach my $rEntity (@$response) {
		...
		for ( my $i = 0 ; $i < @$rEntity ; $i++ ) {
			...
			for ( my $j = $i + 1 ; $j < @$rEntity ; $j++ ) {
				my $id_j = $rEntity->[$j];
				my $temp = CheckEqualityOfEntityInstances(
					$allowMultiple,
					$kIndex->{ Definedm( $id_i, %$kIndex ) },
					$kIndex->{ Definedm( $id_j, %$kIndex ) }
				);
				if ($temp != 0){
					$correct += $temp;
					last;
...
sub BCUBED {
	...
	foreach my $rChain (@$response) {
		foreach my $m (@$rChain) {
			my $kChain =
			  ( defined( $kIndex->{$m} ) ) ? $keys->[ $kIndex->{$m} ] : [];
			my $ci = 0;
			my $ri = scalar(@$rChain);
			my $ki = scalar(@$kChain);
		...
		foreach my $m (@$rChain) {
			...
			if ( defined( $kIndex->{$m} ) ) {
				...
				for my $v ( keys $kIndex ) {
					    # if key has multi-tagged mentions											#if response has multi-tag mentions
...
sub CEAF {
	my ( $keys, $response, $type, $allowMultiple ) = @_;
		...
		for ( my $j = 0 ; $j < scalar(@$response) ; $j++ ) {
			if ( defined( $keys->[$i] ) && defined( $response->[$j] ) ) {
				if ( $type == 0 ) {    # entity-based
					$sim[$i][$j] =
					  1 - SIMEntityBased( $keys->[$i], $response->[$j] );

					# 1 - X => the library searches minima not maxima
				}
				elsif ( $type == 1 ) {    # mention-based
					$sim[$i][$j] =
					  1 - SIMMentionBased( $keys->[$i], $response->[$j] );
				}
			}
			else {
				$sim[$i][$j] = 1;
...
sub SIMEntityBased {
	...
	#find overlap add that to intersection
...
# NEW
sub ScoreBLANC {
	my ( $kFile,   $rFile,   $name,    $allowMultipleTagging ) = @_;
	my ( $acumNRa, $acumDRa, $acumNPa, $acumDPa )              = ( 0, 0, 0, 0 );
	my ( $acumNRr, $acumDRr, $acumNPr, $acumDPr )              = ( 0, 0, 0, 0 );
	my %idenTotals =
	  ( recallDen => 0, recallNum => 0, precisionDen => 0, precisionNum => 0 );

	my $allowMultiple = ( $allowMultipleTagging == 1 )
	  && defined($allowMultipleTagging);
...
"---------------------------------------------------------------------------------"
	...
	for my $rrr (@$response) {
		...
		while ( my @zcombo = $ccombinat->next_combination ) {
			...
			$response_coreference_links->{ $zzcombo[0] . "-" . $zzcombo[1] } =
			  1;
		}
...
"number of links in the intersection of key and response non-coreference links: "
	...
	print "number of key coreference links: "
	  . $num_key_coreference_links . "\n"
	  if ( $VERBOSE > 2 );
...
"--------------------------------------------------------------------------------\n"
	...
	elsif ($num_key_coreference_links == 0
		|| $num_key_non_coreference_links == 0 )
	{
		if ( $num_key_coreference_links == 0 ) {
			$r_blanc = $kncl_recall;
